...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        setting_file = 'setting_size.ini'
    else:
        setting_file = sys.argv[1]
        assert os.path.exists(setting_file), "ファイルが存在しません"

    limit = configparser.ConfigParser() # ２段目の構造重量の下限読み込み専用
    limit.read(setting_file, encoding='utf8') # ２段目の構造重量の下限読み込み専用
        
    rocket = Rocket(setting_file)
    prop0 = [1000, 100]  # 初期値のおまじない
    bounds = [[1, np.inf]] * len(rocket.stages)  # 推進剤の上限下限設定のおまじない
    mass_limit_2nd = limit.getfloat("2段","2段目重量下限値[kg]") # ２段目の構造重量の下限
    cons = ({'type': 'eq',
